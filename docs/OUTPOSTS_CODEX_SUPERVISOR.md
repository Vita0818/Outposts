...
- **Agent 模式**：Codex 本体是主管（supervisor），负责启动/握手/调度/监测/汇总，不直接改目标代码。
- **Spark 模式**：Codex 本体是执行者（implementer），在确认模型后可直接读写目标项目并完成代码修改。
- 两种角色不可混用；禁止在同一轮把 `CODEX` 同时当主管和实现者处理同一轮任务。
- Spark 模式执行前必须确认 `GPT-5.3-Codex-Spark`，否则不得开始。
- 用户反馈优先级高于模型或系统自评，冲突时以用户判定为准。
- 用户命令未明确模式时必须先询问，不得默认执行。
