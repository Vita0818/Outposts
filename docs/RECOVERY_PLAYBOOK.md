...
本文只适用于 Codex 模式：Agent 与 Spark。OpenCode 模式不读取本文，不使用 Codex 恢复机制。
...
若外部后台显示的模型与 DeepCode 握手不一致，或后台计费指标显示非预期模型消耗：

1. 停止启动新轮。
2. 对所有未开始正式轮的项目暂停。
3. 对运行中项目等待自然报告或用户指示。
4. 记录后台观察与 DeepCode 握手模型。
...
如果发现 DeepCode CLI 或 Spark 尝试或已经写入 Apple 源项目：
...
4. 标记 `TOOLCHAIN_MISSING`、`HOST_ENV_BLOCKED` 或 `WINDOWS_HOST_VALIDATION_PENDING`。
...
7. 不执行全局工具链修复。
8. 环境问题标记 `TOOLCHAIN_MISSING`、`HOST_ENV_BLOCKED` 或 `TOOLCHAIN_REPAIR_NEEDS_USER`。
9. 只允许在对应 Outposts 目标项目目录内修改源码和项目配置。

## 视觉证据被删除或无效

如果发现执行器删除当前批次截图、qwen 输出、state、checkpoint、report、batch state 或 `.outposts-supervisor/visual-evidence`：
