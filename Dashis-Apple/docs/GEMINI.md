...
本文是 docs 级 Gemini 入口 shim。Gemini 在本项目中只作为审查副驾驶。
...
- 只读审查项目文件。
- 只能写 `../gemini-report/` 下的 Markdown 或文本报告。
...
- Gemini 报告只能写入 `../gemini-report/`。
- 报告文件名必须采用 `MM_DD_YY-HH_MM-xxxx.md`。
- 报告正文必须先写 `MODEL_CHECK_RESULT`。模型字段只用于记录，不因模型版本号不匹配而停止审查；若项目根入口另有更严格规则，采用更严格规则。
- 只读审查时 `FILES_WRITTEN` 只列本报告。
