...
data class AppProviderAPIKeySource(
    ...
    companion object {
        fun environment(name: String): AppProviderAPIKeySource = AppProviderAPIKeySource(type = "env", value = name)
        fun file(path: String): AppProviderAPIKeySource = AppProviderAPIKeySource(type = "file", value = path)
        fun authFile(): AppProviderAPIKeySource = AppProviderAPIKeySource(type = "authFile", value = "")
        fun providerConfig(path: String): AppProviderAPIKeySource = AppProviderAPIKeySource(type = "providerConfig", value = path)
    }
...
data class AppProviderCatalog(
    ...
    val selectedModel: ProviderModel?
        get() = selectedProvider?.models?.firstOrNull { it.id == selectedModelID } ?: selectedProvider?.models?.firstOrNull()
}
...
data class IntatisConfig(
    ...
    val providerModelCatalog: Map<String, List<ProviderModel>>
        get() {
            ...
            return providerIds.distinct().associate { providerId ->
                providerId.trim().lowercase(Locale.ROOT) to listOf(defaultModel)
            }
        }
