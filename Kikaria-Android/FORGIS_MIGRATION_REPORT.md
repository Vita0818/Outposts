...
## Structured Translation Pass — 2026 Update

### Newly Translated

1. **KikariaTypography system** — Extracted `ui/theme/KikariaTypography.kt` with named font sizes, style builders, and mixed Chinese/serif `AnnotatedString` support matching iOS `KikariaTypography.swift`.

2. **All 4 built-in presets** — `SamplePresets.kt` now contains all 4 source presets (微积分, 离散数学, 大学物理, 大学英语Band4) with real content from source `.md` files.

3. **Preset selection screen** — New `ui/presets/PresetSelectionScreen.kt` with glass-styled cards showing preset name, category, knowledge point count, and active indicator.

4. **Countdown & progress warning** — ViewModel now has `countdownEndDate`, `dangerPercent`, `countdownDays`, `countdownText`, and `StudyProgressWarning` evaluation logic matching iOS.

5. **Review screen enhancements** — Point counter display, previous/next navigation buttons, full dark mode support.

6. **KnowledgePreset extensions** — `knowledgePointCount` computed property and `BUILT_IN_SEED_VERSION` schema tracking.

### Updated Gaps (Previously Listed)

- **No persistent storage** — Still in-memory only. Highest priority next step.
- **No LaTeX/math rendering** — Still deferred.
- **No swipe gestures** — Button-based interaction only.
- **No tablet-adaptive layout** — Still single-column only.
- **No preset management UI** — Selection only; no create/edit/delete/import.
- **No today overview or review history** — Not yet implemented.
- **No study notifications** — Not yet implemented.
- **No widget** — Not yet implemented.
- **No user profile / onboarding** — Not yet implemented.

### Newly Resolved

- ✅ Typography system extracted (was gap #11)
- ✅ All 4 built-in presets available (was partial)
- ✅ Preset selection wired through navigation (was TODO in home screen)
- ✅ Countdown data exposed from ViewModel (was placeholder "-- Days Left")
- ✅ Review screen has previous navigation (was missing)
