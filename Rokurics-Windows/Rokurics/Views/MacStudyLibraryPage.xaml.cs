...
public sealed partial class MacStudyLibraryPage : Page
{
    ...
    /// <summary>
    /// Resolve filing candidates for a given level based on current selections.
    /// Mirrors StudyFilingCandidateResolver from Apple source.
    /// </summary>
    private List<string> ResolveCandidates(string levelKey,
        string? selectedType, string? selectedSubject, string? selectedChapter)
    {
        _studyStore.Refresh();
        var allItems = _studyStore.AllStudyItems
            .Where(i => !i.IsTrashed && i.FilingPath is not null)
            .ToList();

        return levelKey switch
        {
            "type" => allItems
                .Where(i => !string.IsNullOrEmpty(i.FilingPath?.Type))
                .Select(i => i.FilingPath!.Type!)
                .Distinct().OrderBy(t => t).ToList(),
            "subject" => allItems
                .Where(i => i.FilingPath?.Type == selectedType
                    && !string.IsNullOrEmpty(i.FilingPath?.Subject))
                .Select(i => i.FilingPath!.Subject!)
                .Distinct().OrderBy(s => s).ToList(),
            "chapter" => allItems
                .Where(i => i.FilingPath?.Type == selectedType
                    && i.FilingPath?.Subject == selectedSubject
                    && !string.IsNullOrEmpty(i.FilingPath?.Chapter))
                .Select(i => i.FilingPath!.Chapter!)
                .Distinct().OrderBy(c => c).ToList(),
            "topic" => allItems
                .Where(i => i.FilingPath?.Type == selectedType
                    && i.FilingPath?.Subject == selectedSubject
                    && i.FilingPath?.Chapter == selectedChapter
                    && !string.IsNullOrEmpty(i.FilingPath?.Topic))
                .Select(i => i.FilingPath!.Topic!)
                .Distinct().OrderBy(t => t).ToList(),
            _ => new List<string>()
        };
    }
    ...
    /// <summary>
    /// Refresh candidate lists for all 4 levels when a text box changes.
    /// </summary>
    private void RefreshFilingCandidates()
    {
        var selectedType = TypeBox.Text.NullIfEmpty();
        var selectedSubject = SubjectBox.Text.NullIfEmpty();
        var selectedChapter = ChapterBox.Text.NullIfEmpty();

        TypeCandidatesList.ItemsSource = ResolveCandidates("type", null, null, null);
        SubjectCandidatesList.ItemsSource = ResolveCandidates("subject", selectedType, null, null);
        ChapterCandidatesList.ItemsSource = ResolveCandidates("chapter", selectedType, selectedSubject, null);
        TopicCandidatesList.ItemsSource = ResolveCandidates("topic", selectedType, selectedSubject, selectedChapter);
    }

    private void FilingTextBox_TextChanged(object sender, TextChangedEventArgs e)
    {
        RefreshFilingCandidates();
    }

    private void CandidateItem_Tapped(object sender, Microsoft.UI.Xaml.Input.TappedRoutedEventArgs e)
    {
        if (sender is not ListView listView || listView.SelectedItem is not string value) return;

        if (listView == TypeCandidatesList) TypeBox.Text = value;
        else if (listView == SubjectCandidatesList) SubjectBox.Text = value;
        else if (listView == ChapterCandidatesList) ChapterBox.Text = value;
        else if (listView == TopicCandidatesList) TopicBox.Text = value;
